...
struct ImGuiViewportDataDx11
{
	...
	DEBUG_TextureHandle tex_handle;
    ImGuiViewportDataDx11()     { SwapChain = NULL; RTView = NULL; tex_handle = {};}
    ~ImGuiViewportDataDx11()    { IM_ASSERT(SwapChain == NULL && RTView == NULL); }
};

DEBUG_TextureHandle font_tex_handle = {};
//DEBUG_TextureHandle texture_handles[16] = {};

void DEBUGReadFileToBuffer(const char* file_path, unsigned char** buffer, unsigned int* size)
{
    *size = 0;
    *buffer = 0;
	...
	if (GetFileSizeEx(handle, &file_size))
	{
		...
		*buffer = (unsigned char*)malloc(*size); // @malloc
		DWORD dummy;
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
	...
	assert(draw_data->OwnerViewport);
	DEBUG_TextureHandle tex_handle = {};
	ImGuiViewportDataDx11* viewport_user_data = (ImGuiViewportDataDx11*)draw_data->OwnerViewport->RendererUserData;
	if (viewport_user_data)
	{
		tex_handle = viewport_user_data->tex_handle;
	}
	
    {
		...
		VERTEX_CONSTANT_BUFFER buf = 
		{
			...
			{tex_handle.int_vals[0], tex_handle.int_vals[1], tex_handle.int_vals[2], tex_handle.int_vals[3]},
			{tex_handle.float_vals[0], tex_handle.float_vals[1], tex_handle.float_vals[2], tex_handle.float_vals[3]}
		};
        float mvp[4][4] =
        {
            { 2.0f/(R-L),   0.0f,           0.0f,       0.0f },
		...
		memcpy(mapped_resource.pData, &buf, sizeof(buf));
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
				DEBUG_TextureHandle* tex_handle = (DEBUG_TextureHandle*)pcmd->TextureId;
				assert(tex_handle);
				ID3D11ShaderResourceView* texture_srv = (ID3D11ShaderResourceView*)tex_handle->srv;
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
		...
		static const char c_szName[] = "My Cool Texture";
		g_pFontTextureView->SetPrivateData( WKPDID_D3DDebugObjectName, sizeof( c_szName ) - 1, c_szName );
		
        pTexture->Release();
    }
	
    // Store our identifier
	font_tex_handle.srv = g_pFontTextureView;
    io.Fonts->SetTexID((ImTextureID)&font_tex_handle);
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
		...
		unsigned int size = 0;
		DEBUGReadFileToBuffer("shaders/vertex.hlsl", &buffer, &size);
...
float4 float_vals;\
            ...
            struct PS_INPUT\
            {\
uint4 int_vals : INT_VALS;\
float4 float_vals : FLOAT_VALS;\
            ...
            {\
              ...
              output.uv  = input.uv;\
output.int_vals = int_vals;\
output.float_vals = float_vals;\
              return output;\
            }";
		
        ID3DBlob* vertexShaderBlob;
		ID3DBlob* err_blob;
        if (FAILED(D3DCompile(buffer, size, NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, &err_blob)))
		{
    ...
    // Create the pixel shader
    {
		unsigned char* buffer = 0;
		unsigned int size = 0;
		DEBUGReadFileToBuffer("shaders/pixel.hlsl", &buffer, &size);
        ...
        if (FAILED(D3DCompile(buffer, size, NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, &err_blob)))
		{
			OutputDebugStringA((char*)err_blob->GetBufferPointer());
			err_blob->Release();
			free(buffer);
